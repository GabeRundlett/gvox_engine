...
f32vec3 mandelbrot_colored(f32vec2 pixel_p) {
    f32vec2 uv = pixel_p / f32vec2(push_constant.gpu_input.frame_dim.xy);
    uv = (uv - 0.5) * f32vec2(f32(push_constant.gpu_input.frame_dim.x) / f32(push_constant.gpu_input.frame_dim.y), 1);
    f32 time = push_constant.gpu_input.time;
    f32 scale = push_constant.gpu_input.zoom;
    f32vec2 center = push_constant.gpu_input.view_origin;
    u32 max_steps = push_constant.gpu_input.max_steps;
...
void main() {
    ...
    if (pixel_i.x >= push_constant.gpu_input.frame_dim.x || pixel_i.y >= push_constant.gpu_input.frame_dim.y)
        return;
