...
void chunk_opt_x2x4(in u32 chunk_index, in u32 chunk_local_workgroup) {
    ...
    u32 base_id_x1 = sample_voxel_id_BRUSH(chunk_index, in_chunk_i);
    for (i32 x = 0; x < 2; ++x)
        for (i32 y = 0; y < 2; ++y)
            for (i32 z = 0; z < 2; ++z) {
                i32vec3 local_i = i32vec3(in_chunk_i) + i32vec3(x, y, z); // in x1 space
                at_least_one_occluding = at_least_one_occluding || (sample_voxel_id_BRUSH(chunk_index, local_i) != base_id_x1);
            }
    u32 result = 0;
    ...
    x2_i = x4_i * 2;
    u32 base_id_x2 = sample_voxel_id_BRUSH(chunk_index, x2_i * 2);
    ...
    for (i32 x = 0; x < 2; ++x)
        for (i32 y = 0; y < 2; ++y)
            for (i32 z = 0; z < 2; ++z) {
                ...
                at_least_one_occluding = at_least_one_occluding || is_occluding || (sample_voxel_id_BRUSH(chunk_index, local_i * 2) != base_id_x2);
            }
    result = 0;
    if (at_least_one_occluding) {
    ...
    if ((gl_SubgroupInvocationID & 0xF /* = %16 */) == 0) {
        ...
        VOXEL_BRUSH.uniformity_chunks[chunk_index].lod_x4[index] = result;
    }
}

layout(local_size_x = 512, local_size_y = 1, local_size_z = 1) in;
void main() {
    u32 chunk_index = VOXEL_BRUSH.chunk_update_indices[gl_WorkGroupID.y / 64];
...
void chunk_opt_x8up(in u32 chunk_index) {
    ...
    u32 base_id_x4 = sample_voxel_id_BRUSH(chunk_index, x4_i * 4);
    ...
    for (i32 x = 0; x < 2; ++x)
        for (i32 y = 0; y < 2; ++y)
            for (i32 z = 0; z < 2; ++z) {
                ...
                u32 mask = uniformity_lod_mask(local_i);
                b32 occluding = (VOXEL_BRUSH.uniformity_chunks[chunk_index].lod_x4[index] & mask) != 0;
                at_least_one_occluding = at_least_one_occluding || occluding || (sample_voxel_id_BRUSH(chunk_index, local_i * 4) != base_id_x4);
            }
    ...
    if ((gl_SubgroupInvocationID & 0x7 /* == % 8*/) == 0) {
        ...
        VOXEL_BRUSH.uniformity_chunks[chunk_index].lod_x8[index] = result;
    ...
    x8_i = x16_i * 2;
    u32 base_id_x8 = sample_voxel_id_BRUSH(chunk_index, x8_i * 8);
    ...
    for (i32 x = 0; x < 2; ++x)
        for (i32 y = 0; y < 2; ++y)
            for (i32 z = 0; z < 2; ++z) {
                ...
                at_least_one_occluding = at_least_one_occluding || is_occluding || (sample_voxel_id_BRUSH(chunk_index, local_i * 8) != base_id_x8);
            }
    ...
    if ((gl_SubgroupInvocationID & 0x3) == 0) {
        ...
        VOXEL_BRUSH.uniformity_chunks[chunk_index].lod_x16[index] = result;
    ...
    x16_i = x32_i * 2;
    u32 base_id_x16 = sample_voxel_id_BRUSH(chunk_index, x16_i * 16);
    ...
    for (i32 x = 0; x < 2; ++x)
        for (i32 y = 0; y < 2; ++y)
            for (i32 z = 0; z < 2; ++z) {
                ...
                at_least_one_occluding = at_least_one_occluding || is_occluding || (sample_voxel_id_BRUSH(chunk_index, local_i * 16) != base_id_x16);
            }
    ...
    if ((gl_SubgroupInvocationID & 0x1) == 0) {
        ...
        VOXEL_BRUSH.uniformity_chunks[chunk_index].lod_x32[index] = result;
    }
}

layout(local_size_x = 512, local_size_y = 1, local_size_z = 1) in;
void main() {
    u32 chunk_index = VOXEL_BRUSH.chunk_update_indices[gl_WorkGroupID.y];
