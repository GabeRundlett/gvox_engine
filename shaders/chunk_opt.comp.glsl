...
void chunk_opt_x2x4(in u32 chunk_index, in u32 chunk_local_workgroup) {
    ...
    u32 base_id_x1 = sample_voxel_id_WORLD(chunk_index, in_chunk_i);
    for (i32 x = 0; x < 2; ++x)
        for (i32 y = 0; y < 2; ++y)
            for (i32 z = 0; z < 2; ++z) {
                i32vec3 local_i = i32vec3(in_chunk_i) + i32vec3(x, y, z); // in x1 space
                at_least_one_occluding = at_least_one_occluding || (sample_voxel_id_WORLD(chunk_index, local_i) != base_id_x1);
            }
    u32 result = 0;
    ...
    if ((gl_SubgroupInvocationID & 0x1F /* = %32 */) == 0) {
        ...
        VOXEL_WORLD.uniformity_chunks[chunk_index].lod_x2[index] = result;
    ...
    x2_i = x4_i * 2;
    u32 base_id_x2 = sample_voxel_id_WORLD(chunk_index, x2_i * 2);
    ...
    for (i32 x = 0; x < 2; ++x)
        for (i32 y = 0; y < 2; ++y)
            for (i32 z = 0; z < 2; ++z) {
                ...
                at_least_one_occluding = at_least_one_occluding || is_occluding || (sample_voxel_id_WORLD(chunk_index, local_i * 2) != base_id_x2);
            }
    result = 0;
    if (at_least_one_occluding) {
...
void chunk_opt_x8up(in u32 chunk_index) {
    ...
    u32 base_id_x4 = sample_voxel_id_WORLD(chunk_index, x4_i * 4);
    ...
    for (i32 x = 0; x < 2; ++x)
        for (i32 y = 0; y < 2; ++y)
            for (i32 z = 0; z < 2; ++z) {
                ...
                at_least_one_occluding = at_least_one_occluding || occluding || (sample_voxel_id_WORLD(chunk_index, local_i * 4) != base_id_x4);
            }
    ...
    x8_i = x16_i * 2;
    u32 base_id_x8 = sample_voxel_id_WORLD(chunk_index, x8_i * 8);
    ...
    for (i32 x = 0; x < 2; ++x)
        for (i32 y = 0; y < 2; ++y)
            for (i32 z = 0; z < 2; ++z) {
                ...
                at_least_one_occluding = at_least_one_occluding || is_occluding || (sample_voxel_id_WORLD(chunk_index, local_i * 8) != base_id_x8);
            }
    ...
    x16_i = x32_i * 2;
    u32 base_id_x16 = sample_voxel_id_WORLD(chunk_index, x16_i * 16);
    ...
    for (i32 x = 0; x < 2; ++x)
        for (i32 y = 0; y < 2; ++y)
            for (i32 z = 0; z < 2; ++z) {
                ...
                at_least_one_occluding = at_least_one_occluding || is_occluding || (sample_voxel_id_WORLD(chunk_index, local_i * 16) != base_id_x16);
            }
