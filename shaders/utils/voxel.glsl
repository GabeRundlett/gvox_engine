...
PackedVoxel pack_voxel(in Voxel voxel) {
    ...
    result.data = bitpacking_pack_f32vec3(12, 4, result.data, bitpacking_map_range(f32vec3(-1), f32vec3(1), voxel.col));
    result.data = bitpacking_pack_u32(24, 8, result.data, voxel.mat_id);
    return result;
}
Voxel unpack_voxel(in PackedVoxel packed_voxel) {
    ...
    result.mat_id = bitpacking_unpack_u32(24, 8, packed_voxel.data);
    return result;
}
