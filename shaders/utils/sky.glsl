...
f32 optical_depth_baked(Ray ray) {
    ...
    return texture(sampler2D(daxa_GetImage(texture2D, push_constant.optical_depth_image_id), daxa_GetSampler(sampler, push_constant.optical_depth_sampler_id)), f32vec2(angle, height_normalized)).r;
}
