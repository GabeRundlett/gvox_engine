...
f32vec2 prev_uv_from_pos(f32 aspect, f32 fov, f32vec3 pos) {
    f32vec3 dir = normalize(pos - deref(globals).player.cam.prev_pos) * deref(globals).player.cam.prev_rot_mat;
    return dir.xy / dir.z * f32vec2(0.5 / aspect, 0.5) / deref(globals).player.cam.prev_tan_half_fov;
}
...
f32vec4 get_prev_sample(i32vec2 prev_pixel_i, f32vec3 hit) {
    if (min(i32vec2(frame_dim), prev_pixel_i) != prev_pixel_i || max(i32vec2(0, 0), prev_pixel_i) != prev_pixel_i)
        return f32vec4(f32vec3(0), MAX_DIST);
    ...
    f32vec3 view_dir = create_view_dir(globals, (uv * 2.0 - 1.0) * vec2(aspect, 1.0));
