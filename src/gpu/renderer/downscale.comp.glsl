...
void main() {
    ...
    daxa_f32vec4 output_val = texelFetch(daxa_texture2D(src_image_id), src_px, 0);
#elif DOWNSCALE_NRM
    daxa_u32vec4 g_buffer_value = texelFetch(daxa_utexture2D(src_image_id), src_px, 0);
    ...
    imageStore(daxa_image2D(dst_image_id), daxa_i32vec2(gl_GlobalInvocationID.xy), output_val);
}
