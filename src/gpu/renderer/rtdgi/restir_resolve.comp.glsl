...
[[vk::binding(0)]] Texture2D<vec4> radiance_tex;
[[vk::binding(1)]] Texture2D<uvec2> reservoir_input_tex;
[[vk::binding(2)]] Texture2D<vec4> gbuffer_tex;
[[vk::binding(3)]] Texture2D<float> depth_tex;
[[vk::binding(4)]] Texture2D<vec4> half_view_normal_tex;
[[vk::binding(5)]] Texture2D<float> half_depth_tex;
[[vk::binding(6)]] Texture2D<vec4> ssao_tex;
[[vk::binding(7)]] Texture2D<vec4> candidate_radiance_tex;
[[vk::binding(8)]] Texture2D<vec4> candidate_hit_tex;
[[vk::binding(9)]] Texture2D<uvec4> temporal_reservoir_packed_tex;
[[vk::binding(10)]] Texture2D<vec3> bounced_radiance_input_tex;
[[vk::binding(11)]] RWTexture2D<vec4> irradiance_output_tex;
...
void main(uvec2 px : SV_DispatchThreadID) {
    ...
    const vec2 uv = get_uv(px, gbuffer_tex_size);
    ...
    const vec3 center_normal_vs = direction_world_to_view(center_normal_ws);
    ...
    const vec4 blue = blue_noise_for_pixel(px, frame_constants.frame_index) * M_TAU;
    ...
    vec3 total_irradiance = 0;
    bool sharpen_gi_kernel = false;

    {
    float w_sum = 0;
    vec3 weighted_irradiance = 0;
    ...
    for (uint sample_i = 0; sample_i < select(RTDGI_RESTIR_USE_RESOLVE_SPATIAL_FILTER, 4, 1); ++sample_i) {
        ...
        if (USE_SPLIT_RT_NEAR_FIELD) {
            const vec3 hit_ws = candidate_hit_tex[rpx].xyz + rpx_ray_ctx.ray_hit_ws();
            const vec3 sample_offset = hit_ws - view_ray_context.ray_hit_ws();
            ...
            vec3 contribution = candidate_radiance_tex[rpx].rgb * geometric_term;
            contribution *= lerp(0.0, atten, near_field_influence);

            vec3 sample_normal_vs = half_view_normal_tex[rpx].rgb;
    ...
    for (uint sample_i = 0; sample_i < select(RTDGI_RESTIR_USE_RESOLVE_SPATIAL_FILTER, 4, 1); ++sample_i) {
...

        {
            ...
            const vec3 hit_ws = spx_packed.ray_hit_offset_ws + spx_ray_ctx.ray_hit_ws();
            const vec3 sample_offset = hit_ws - view_ray_context.ray_hit_ws();
            const float sample_dist = length(sample_offset);
            const vec3 sample_dir = sample_offset / sample_dist;
            ...
            vec3 sample_normal_vs = half_view_normal_tex[spx].rgb;
            const float sample_ssao = ssao_tex[rpx * 2 + HALFRES_SUBSAMPLE_OFFSET].r;
    ...
    irradiance_output_tex[px] = vec4(total_irradiance, 1);
}
