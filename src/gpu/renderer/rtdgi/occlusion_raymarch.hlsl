struct OcclusionScreenRayMarch {
    ...
    Texture2D<vec3> fullres_color_bounce_tex;

    OcclusionScreenRayMarch with_color_bounce(Texture2D<vec3> _fullres_color_bounce_tex) {
    ...
    void march(
        ...
        const vec2 raymarch_end_uv = cs_to_uv(position_world_to_clip(raymarch_end_ws).xy);
        const vec2 raymarch_uv_delta = raymarch_end_uv - raymarch_start_uv;
        const vec2 raymarch_len_px = raymarch_uv_delta * select(use_halfres_depth, halfres_depth_tex_size, fullres_depth_tex_size);
        ...
        const vec3 raymarch_end_cs = position_world_to_clip(raymarch_end_ws).xyz;
        ...
        for (int k = 0; k < k_count; ++k) {
            const vec3 interp_pos_cs = lerp(raymarch_start_cs, raymarch_end_cs, t);
            ...
            const vec2 quantized_cs_at_interp = uv_to_cs((px_at_interp + 0.5) / fullres_depth_tex_size);
