...
[[vk::binding(0)]] Texture2D<uvec2> reservoir_input_tex;
[[vk::binding(1)]] Texture2D<vec3> bounced_radiance_input_tex;
[[vk::binding(2)]] Texture2D<vec4> half_view_normal_tex;
...
[[vk::binding(6)]] Texture2D<uvec4> temporal_reservoir_packed_tex;
[[vk::binding(7)]] Texture2D<vec3> reprojected_gi_tex;
[[vk::binding(8)]] RWTexture2D<uvec2> reservoir_output_tex;
[[vk::binding(9)]] RWTexture2D<vec3> bounced_radiance_output_tex;
...
void main(uvec2 px : SV_DispatchThreadID) {
    ...
    const vec3 center_normal_vs = half_view_normal_tex[px].rgb;
    const vec3 center_normal_ws = direction_view_to_world(center_normal_vs);
    ...
    const vec2 dist_to_edge_xy = min(vec2(px), output_tex_size.xy - px);
    const float allow_edge_overstep = select(center_r.M < 10, 100.0, 1.25);
    //const float allow_edge_overstep = 1.25;
    const vec2 kernel_radius = min(max_kernel_radius, dist_to_edge_xy * allow_edge_overstep);
    ...
    uint sample_count = select(DIFFUSE_GI_USE_RESTIR
        ...
        const uvec2 ang_offset_seed = select(spatial_reuse_pass_idx == 0
            , (px >> 3)
            , (px >> 2));
    #else
    ...
    float ang_offset = uint_to_u01_float(hash3(
        uvec3(ang_offset_seed, frame_constants.frame_index * 2 + spatial_reuse_pass_idx)
    )) * M_PI * 2;
    ...
    for (uint sample_i = 0; sample_i < sample_count; ++sample_i) {
        ...
        ivec2 rpx_offset = vec2(cos(ang), sin(ang)) * radius;

        const bool is_center_sample = sample_i == 0;
        //const bool is_center_sample = all(rpx_offset == 0);

        const ivec2 rpx = px + rpx_offset;

        const uvec2 reservoir_raw = reservoir_input_tex[rpx];
        ...
        const vec3 sample_normal_vs = half_view_normal_tex[rpx].rgb;
        ...
        const vec3 sample_hit_ws = spx_packed.ray_hit_offset_ws + spx_ray_ctx.ray_hit_ws();

        const vec3 reused_dir_to_sample_hit_unnorm_ws = sample_hit_ws - rpx_ray_ctx.ray_hit_ws();
        ...
        const vec3 dir_to_sample_hit_unnorm = sample_hit_ws - view_ray_context.ray_hit_ws();
        ...
        if (RTDGI_RESTIR_SPATIAL_USE_RAYMARCH && perform_occlusion_raymarch) {
            ...
            const vec3 raymarch_dir_unnorm_ws = sample_hit_ws - view_ray_context.ray_hit_ws();
