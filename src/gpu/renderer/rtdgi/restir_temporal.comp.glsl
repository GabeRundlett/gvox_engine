...
[[vk::binding(0)]] Texture2D<vec3> half_view_normal_tex;
[[vk::binding(1)]] Texture2D<float> depth_tex;
[[vk::binding(2)]] Texture2D<vec4> candidate_radiance_tex;
[[vk::binding(3)]] Texture2D<vec3> candidate_normal_tex;
[[vk::binding(4)]] Texture2D<vec4> candidate_hit_tex;
[[vk::binding(5)]] Texture2D<vec4> radiance_history_tex;
[[vk::binding(6)]] Texture2D<vec3> ray_orig_history_tex;
[[vk::binding(7)]] Texture2D<vec4> ray_history_tex;
[[vk::binding(8)]] Texture2D<uvec2> reservoir_history_tex;
[[vk::binding(9)]] Texture2D<vec4> reprojection_tex;
[[vk::binding(10)]] Texture2D<vec4> hit_normal_history_tex;
[[vk::binding(11)]] Texture2D<vec4> candidate_history_tex;
[[vk::binding(12)]] Texture2D<vec2> rt_invalidity_tex;
[[vk::binding(13)]] RWTexture2D<vec4> radiance_out_tex;
[[vk::binding(14)]] RWTexture2D<vec3> ray_orig_output_tex;
[[vk::binding(15)]] RWTexture2D<vec4> ray_output_tex;
[[vk::binding(16)]] RWTexture2D<vec4> hit_normal_output_tex;
[[vk::binding(17)]] RWTexture2D<uvec2> reservoir_out_tex;
[[vk::binding(18)]] RWTexture2D<vec4> candidate_out_tex;
[[vk::binding(19)]] RWTexture2D<uvec4> temporal_reservoir_packed_tex;
...
TraceResult do_the_thing(uvec2 px, inout uint rng, RayDesc outgoing_ray, vec3 primary_hit_normal) {
    const vec4 candidate_radiance_inv_pdf = candidate_radiance_tex[px];
    TraceResult result;
...
void main(uvec2 px : SV_DispatchThreadID) {
    ...
    if (0.0 == depth) {
        radiance_out_tex[px] = vec4(0.0.xxx, -SKY_DIST);
    ...
    const vec3 hit_offset_ws = candidate_hit_tex[px].xyz;
    ...
    if (is_rtdgi_tracing_frame()) {
        ...
        candidate_out_tex[px] = vec4(sqrt(hit_t), rl, 0, 0);
    }
    ...
    if (use_resampling) {
        for (
            ...
            ++sample_i) {
            const ivec2 rpx_offset = get_rpx_offset(sample_i, frame_constants.frame_index);
            if (sample_i > 0 && all(rpx_offset == 0)) {
                // No point using the center sample twice
                continue;
            }

            const vec4 reproj = reprojection_tex[hi_px + rpx_offset * 2];
            ...
            // Or not at all.
            const vec2 reproj_rand_offset = 0.0;
            ...
            const ivec2 permuted_reproj_px = floor(
                select(sample_i == 0
                    , px
            ...
            const ivec2 permuted_neighbor_px = floor(
                select(sample_i == 0
                    , px
                    // ditto
            ...
            const vec3 prev_ray_orig = ray_orig_history_tex[spx];
            ...
            const vec3 sample_normal_vs = half_view_normal_tex[neighbor_px].rgb;
        ...
        // Increases noise, but prevents leaking in areas of geometric complexity
            ...
            const vec4 sample_hit_ws_and_dist = ray_history_tex[spx] + vec4(prev_ray_orig, 0.0);
    ...
    radiance_out_tex[px] = vec4(radiance_sel, dot(normal_ws, outgoing_ray.Direction));
    ray_orig_output_tex[px] = ray_orig_sel_ws;
    hit_normal_output_tex[px] = encode_hit_normal_and_dot(hit_normal_ws_dot);
    ray_output_tex[px] = vec4(ray_hit_sel_ws - ray_orig_sel_ws, length(ray_hit_sel_ws - refl_ray_origin_ws));
