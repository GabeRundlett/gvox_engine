...
[[vk::binding(3)]] Texture2D<vec4> half_view_normal_tex;
[[vk::binding(4)]] Texture2D<float> half_depth_tex;
[[vk::binding(5)]] RWTexture2D<vec2> output_tex;
...
[numthreads(8, 8, 1)]
void main(in uvec2 px : SV_DispatchThreadID) {
    vec2 invalid_blurred = 0;

    const float center_depth = half_depth_tex[px];
    const vec4 reproj = reprojection_tex[px * 2];
    ...
    if (RESTIR_USE_PATH_VALIDATION) {
        ...
        for (int y = -k; y <= k; ++y) {
            for (int x = -k; x <= k; ++x) {
                ...
                invalid_blurred += vec2(input_tex[px + offset], 1) * w;
            }
        }}
    ...
    for (int y = 0; y <= k; ++y) {
        for (int x = 1; x <= k; ++x) {
            const ivec2 offset = ivec2(x, y);
            const ivec2 sample_px = px * 2 + offset;
            const ivec2 sample_px_half = px + offset / 2;
            const vec4 reproj = reprojection_tex[sample_px];
    ...
    const vec2 reproj_px = px + gbuffer_tex_size.xy * reproj.xy / 2 + 0.5;
    float history = 0;

    const int sample_count = 8;
    float ang_off = uint_to_u01_float(hash3(uvec3(px, frame_constants.frame_index))) * M_PI * 2;
