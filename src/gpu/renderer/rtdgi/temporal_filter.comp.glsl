...
[[vk::binding(2)]] Texture2D<vec2> variance_history_tex;
[[vk::binding(3)]] Texture2D<vec4> reprojection_tex;
[[vk::binding(4)]] Texture2D<vec2> rt_history_invalidity_tex;
[[vk::binding(5)]] RWTexture2D<vec4> output_tex;
[[vk::binding(6)]] RWTexture2D<vec4> history_output_tex;
[[vk::binding(7)]] RWTexture2D<vec2> variance_history_output_tex;
...
void main(uvec2 px: SV_DispatchThreadID) {
        ...
        history_output_tex[px] = vec4(max(0.0, input_tex[px].rgb), 32);
        return;
    #endif

    vec2 uv = get_uv(px, output_tex_size);
    
    vec4 center = linear_to_working(input_tex[px]);
    vec4 reproj = reprojection_tex[px];

    const vec4 history_mult = vec4((frame_constants.pre_exposure_delta).xxx, 1);
    vec4 history = linear_to_working(history_tex[px] * history_mult);
    ...
    {for (int y = -k; y <= k; ++y) {
        for (int x = -k; x <= k; ++x) {
            ...
            vec4 hist_neigh = linear_to_working(history_tex[px + ivec2(x, y)] * history_mult);
    ...
    const vec2 moments_history =
        ...
        * vec2(frame_constants.pre_exposure_delta, frame_constants.pre_exposure_delta * frame_constants.pre_exposure_delta);
    ...
    vec3 hist_dev = sqrt(abs(hist_vsum2 - hist_vsum * hist_vsum));
...
//max_sample_count = 32;
    ...
    vec3 res = lerp(clamped_history.rgb, center.rgb, 1.0 / (1.0 + min(max_sample_count, current_sample_count)));
    //vec3 res = lerp(clamped_history.rgb, center.rgb, 1.0 / 32);

    const float output_sample_count = min(current_sample_count, max_sample_count) + 1;
    vec4 output = working_to_linear(vec4(res, output_sample_count));
