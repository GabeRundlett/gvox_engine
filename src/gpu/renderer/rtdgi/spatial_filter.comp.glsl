...
[[vk::binding(2)]] Texture2D<vec4> ssao_tex;
[[vk::binding(3)]] Texture2D<vec3> geometric_normal_tex;
[[vk::binding(4)]] RWTexture2D<vec4> output_tex;
...
void main(in uvec2 px : SV_DispatchThreadID) {
    ...
    const vec3 center_value = input_tex[px].rgb;
    const vec3 center_normal_vs = geometric_normal_tex[px] * 2.0 - 1.0;

    if (center_validity == 1) {
        output_tex[px] = vec4(center_value, 1.0);
        return;
    }
...

    {
        ...
        for (uint sample_i = 1; sample_i < MAX_SAMPLE_COUNT; ++sample_i) {
            ...
            const ivec2 sample_px = px + sample_offset;

            const float sample_depth = depth_tex[sample_px];
            const vec3 sample_val = input_tex[sample_px].rgb;
            const float sample_ssao = ssao_tex[sample_px].r;
            const vec3 sample_normal_vs = geometric_normal_tex[sample_px] * 2.0 - 1.0;
    ...
    output_tex[px] = vec4(filtered, 1.0);
}
