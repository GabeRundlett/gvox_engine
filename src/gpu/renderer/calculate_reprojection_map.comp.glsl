...
void main() {
    ...
    if (depth == 0.0) {
        daxa_f32vec4 pos_cs = daxa_f32vec4(uv_to_cs(uv), 0.0, 1.0);
        daxa_f32vec4 pos_vs = deref(globals).player.cam.clip_to_view * pos_cs;

        daxa_f32vec4 prev_vs = pos_vs;

        daxa_f32vec4 prev_cs = deref(globals).player.cam.view_to_clip * prev_vs;
        daxa_f32vec4 prev_pcs = deref(globals).player.cam.clip_to_prev_clip * prev_cs;
    ...
    daxa_f32vec3 eye_pos = (deref(globals).player.cam.view_to_world * daxa_f32vec4(0, 0, 0, 1)).xyz;
    ...
    daxa_f32vec4 prev_vs = pos_vs / pos_vs.w;
    prev_vs.xyz += daxa_f32vec4(fetch_vel(px).xyz, 0).xyz;

    // daxa_f32vec4 prev_cs = mul(deref(globals).player.cam.prev_view_to_prev_clip, prev_vs);
    daxa_f32vec4 prev_cs = (deref(globals).player.cam.view_to_clip * prev_vs);
    daxa_f32vec4 prev_pcs = (deref(globals).player.cam.clip_to_prev_clip * prev_cs);

    daxa_f32vec2 prev_uv = cs_to_uv(prev_pcs.xy / prev_pcs.w);
    daxa_f32vec2 uv_diff = prev_uv - uv;
    ...
    daxa_f32vec4 prev_pvs = (deref(globals).player.cam.prev_clip_to_prev_view * prev_pcs);
    prev_pvs /= prev_pvs.w;
    ...
    daxa_f32vec2 prev_gather_uv = (bilinear_at_prev.origin + 1.0) / deref(gpu_input).rounded_frame_dim.xy;
    daxa_f32vec4 prev_depth = textureGather(daxa_sampler2D(prev_depth_image_id, deref(gpu_input).sampler_nnc), prev_gather_uv).wzxy;

    // daxa_f32vec4 prev_view_z = rcp(prev_depth * -deref(globals).player.cam.prev_clip_to_prev_view._43);
    daxa_f32vec4 prev_view_z = 1.0 / (prev_depth * -deref(globals).player.cam.prev_clip_to_prev_view[2][3]);

    // Note: departure from the quoted technique: linear offset from zero distance at previous position instead of scaling.
    daxa_f32vec4 quad_dists = abs(plane_dist_prev_dz * (prev_view_z - prev_pvs.z));
    ...
    const daxa_f32vec3 pos_vs_norm = normalize(pos_vs.xyz / pos_vs.w);
    ...
    daxa_f32vec4 quad_validity = step(quad_dists, daxa_f32vec4((acceptable_dist)));

    daxa_i32vec2 p_x0 = px0(bilinear_at_prev);
    daxa_i32vec2 p_x1 = px1(bilinear_at_prev);
    daxa_i32vec2 p_x2 = px2(bilinear_at_prev);
    daxa_i32vec2 p_x3 = px3(bilinear_at_prev);

    quad_validity.x *= daxa_f32((p_x0.x >= 0 && p_x0.y >= 0) && (p_x0.x < output_tex_size.x && p_x0.y < output_tex_size.y));
    quad_validity.y *= daxa_f32((p_x1.x >= 0 && p_x1.y >= 0) && (p_x1.x < output_tex_size.x && p_x1.y < output_tex_size.y));
    quad_validity.z *= daxa_f32((p_x2.x >= 0 && p_x2.y >= 0) && (p_x2.x < output_tex_size.x && p_x2.y < output_tex_size.y));
    quad_validity.w *= daxa_f32((p_x3.x >= 0 && p_x3.y >= 0) && (p_x3.x < output_tex_size.x && p_x3.y < output_tex_size.y));

    float validity = dot(quad_validity, daxa_f32vec4(1, 2, 4, 8)) / 15.0;
    float accuracy = 1;
