...
[[vk::binding(0)]] Texture2D<vec4> gbuffer_tex;
[[vk::binding(1)]] Texture2D<vec3> half_view_normal_tex;
[[vk::binding(2)]] Texture2D<float> depth_tex;
[[vk::binding(3)]] Texture2D<vec4> candidate0_tex;
[[vk::binding(4)]] Texture2D<vec4> candidate1_tex;
[[vk::binding(5)]] Texture2D<vec4> candidate2_tex;
[[vk::binding(6)]] Texture2D<vec4> irradiance_history_tex;
[[vk::binding(7)]] Texture2D<vec4> ray_orig_history_tex;
[[vk::binding(8)]] Texture2D<vec4> ray_history_tex;
[[vk::binding(9)]] Texture2D<uint> rng_history_tex;
[[vk::binding(10)]] Texture2D<uvec2> reservoir_history_tex;
[[vk::binding(11)]] Texture2D<vec4> reprojection_tex;
[[vk::binding(12)]] Texture2D<vec4> hit_normal_history_tex;
[[vk::binding(13)]] RWTexture2D<vec4> irradiance_out_tex;
[[vk::binding(14)]] RWTexture2D<vec4> ray_orig_output_tex;
[[vk::binding(15)]] RWTexture2D<vec4> ray_output_tex;
[[vk::binding(16)]] RWTexture2D<uint> rng_output_tex;
[[vk::binding(17)]] RWTexture2D<vec4> hit_normal_output_tex;
[[vk::binding(18)]] RWTexture2D<uvec2> reservoir_out_tex;
[[vk::binding(19)]] cbuffer _ {
    vec4 gbuffer_tex_size;
};
...
TraceResult do_the_thing(uvec2 px, vec3 primary_hit_normal) {
    const vec4 hit0 = candidate0_tex[px];
    const vec4 hit1 = candidate1_tex[px];
    const vec4 hit2 = candidate2_tex[px];
...
void find_best_reprojection_in_neighborhood(vec2 base_px, inout ivec2 best_px, vec3 refl_ray_origin_ws, bool wide) {
    ...
    const vec2 clip_scale = frame_constants.view_constants.clip_to_view._m00_m11;
    const vec2 offset_scale = vec2(1, -1) * -2 * clip_scale * gbuffer_tex_size.zw;

    const vec3 look_direction = direction_view_to_world(vec3(0, 0, -1));

    {
        ...
        refl_ray_origin_ws
            += direction_view_to_world(vec3(vec2(HALFRES_SUBSAMPLE_OFFSET) * offset_scale * z_offset, 0));
    }

    const int start_coord = select(wide, -1, 0);
    for (int y = start_coord; y <= 1; ++y) {
        for (int x = start_coord; x <= 1; ++x) {
            ivec2 spx = floor(base_px + vec2(x, y));

            RtrRestirRayOrigin ray_orig = RtrRestirRayOrigin::from_raw(ray_orig_history_tex[spx]);
            vec3 orig = ray_orig.ray_origin_eye_offset_ws + get_prev_eye_position();
...

            {
                ...
                orig += direction_view_to_world(vec3(vec2(orig_jitter) * offset_scale * z_offset, 0));
            }
...
void main(uvec2 px : SV_DispatchThreadID) {
    const ivec2 hi_px_offset = HALFRES_SUBSAMPLE_OFFSET;
    const uvec2 hi_px = px * 2 + hi_px_offset;
    
    float depth = depth_tex[hi_px];

    if (0.0 == depth) {
        irradiance_out_tex[px] = vec4(0.0.xxx, -SKY_DIST);
        hit_normal_output_tex[px] = 0.0.xxxx;
        reservoir_out_tex[px] = 0;
        return;
    }

    const vec2 uv = get_uv(hi_px, gbuffer_tex_size);
    const vec3 normal_vs = half_view_normal_tex[px];
    const vec3 normal_ws = direction_view_to_world(normal_vs);

    float local_normal_flatness = 1; {
        const int k = 1;
        for (int y = -k; y <= k; ++y) {
            for (int x = -k; x <= k; ++x) {
                vec3 sn_vs = half_view_normal_tex[px + ivec2(x, y)];
    ...
    float reprojection_neighborhood_stability = 1;
    {
        for (int y = 0; y <= 1; ++y) {
            for (int x = 0; x <= 1; ++x) {
                float r = reprojection_tex[px * 2 + ivec2(x, y)].z;
    ...
    const vec3 refl_ray_origin_ws = view_ray_context.biased_secondary_ray_origin_ws_with_normal(normal_ws);
#else
    const ViewRayContext view_ray_context = ViewRayContext::from_uv_and_depth(uv, depth);
    const vec3 refl_ray_origin_ws = view_ray_context.biased_secondary_ray_origin_ws();
#endif

    const vec3 refl_ray_origin_vs = position_world_to_view(refl_ray_origin_ws);

    const float3x3 tangent_to_world = build_orthonormal_basis(normal_ws);
    vec3 outgoing_dir = vec3(0, 0, 1);

    uint rng = hash3(uvec3(px, frame_constants.frame_index));
    vec3 wo = mul(-normalize(view_ray_context.ray_dir_ws()), tangent_to_world);
    ...
    const vec4 gbuffer_packed = gbuffer_tex[hi_px];
    ...
    float pdf_sel = 0;
    float cos_theta = 0;
    vec3 irradiance_sel = 0;
    vec4 ray_orig_sel = 0;
    vec3 ray_hit_sel_ws = 1;
    vec3 hit_normal_sel = 1;
...

    {
        ...
        if (result.pdf > 0) {
            ...
            vec3 wi = normalize(mul(outgoing_dir, tangent_to_world));
    ...
    const vec4 center_reproj = reprojection_tex[hi_px];
    ...
    if (use_resampling) {
        ...
        //for (uint sample_i = 0; sample_i < 1; ++sample_i) {
            ...
            //) * 7;
            const vec2 reservoir_px_offset_base = vec2(
                cos(ang), sin(ang)
            ) * rpx_offset_radius;
            ...
            const ivec2 rpx_offset =
                select(sample_i == 0
                , ivec2(0, 0)
                , ivec2(reservoir_px_offset_base))
                ;

            vec4 reproj = reprojection_tex[hi_px + rpx_offset * 2];

            // Can't use linear interpolation.

            const vec2 reproj_px_flt = px + gbuffer_tex_size.xy * reproj.xy / 2;

            ivec2 reproj_px;
            

            {
                const vec2 base_px = px + gbuffer_tex_size.xy * reproj.xy / 2;
                ivec2 best_px = floor(base_px + 0.5);
            ...
            const vec2 sample_uv = get_uv(rpx_hi, gbuffer_tex_size);
            const vec4 prev_ray_orig_and_roughness = ray_orig_history_tex[spx] + vec4(get_prev_eye_position(), 0);
            ...
            const vec4 prev_irrad_and_cos_theta =
                ...
                * vec4((frame_constants.pre_exposure_delta).xxx, 1);
            ...
            const vec4 sample_hit_ws_and_pdf_packed = ray_history_tex[spx];
            ...
            const vec4 sample_hit_normal_ws_dot = decode_hit_normal_and_dot(hit_normal_history_tex[spx]);
            ...
            const vec3 wi = normalize(mul(dir_to_sample_hit, tangent_to_world));
            ...
            if (USE_TRANSLATIONAL_CLAMP) {
                ...
                const vec3 current_wo = normalize(view_ray_context.ray_hit_ws() - get_eye_position());
                const vec3 prev_wo = normalize(view_ray_context.ray_hit_ws() - get_prev_eye_position());
                ...
                float mult = max(wo_similarity, 1, smoothstep(0.05, 0.5, sqrt(gbuffer.roughness)));
                ...
                mult = max(1.0, mult, local_normal_flatness);
            ...
            if (USE_JACOBIAN_BASED_REJECTION) {
                const float JACOBIAN_REJECT_THRESHOLD = max(1.1, 4.0, gbuffer.roughness * gbuffer.roughness);
            ...
            if (USE_DISTANCE_BASED_M_CLAMP) {
                    ...
                    * frame_constants.view_constants.view_to_clip[1][1];
                {
                    ...
                    r.M = min(r.M, RTR_RESTIR_TEMPORAL_M_CLAMP * max(saturate(50.0 * dist2), 1.0, gbuffer.roughness * gbuffer.roughness));
                }
            }
...
// TODO: was `refl_ray_origin_ws`; what should it be?
    ...
    irradiance_out_tex[px] = vec4(irradiance_sel, rtr_encode_cos_theta_for_fp16(cos_theta));
    // Note: relies on the `xyz` being directly encoded by `RtrRestirRayOrigin`
    ray_orig_output_tex[px] = vec4(ray_orig_sel.xyz - get_eye_position(), ray_orig_sel.w);
    hit_normal_output_tex[px] = encode_hit_normal_and_dot(hit_normal_ws_dot);
    ray_output_tex[px] = vec4(ray_hit_sel_ws - ray_orig_sel.xyz, pdf_sel);
