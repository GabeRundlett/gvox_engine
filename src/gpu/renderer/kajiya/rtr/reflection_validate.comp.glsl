...
[[vk::binding(2)]] Texture2D<vec4> rtdgi_tex;
[[vk::binding(3)]] TextureCube<vec4> sky_cube_tex;
...
[[vk::binding(15)]] Texture2D<vec4> ray_orig_history_tex;
[[vk::binding(16)]] Texture2D<vec4> ray_history_tex;
[[vk::binding(17)]] Texture2D<uint> rng_history_tex;
[[vk::binding(18)]] RWTexture2D<vec4> irradiance_history_tex;
[[vk::binding(19)]] RWTexture2D<uvec2> reservoir_history_tex;
...
void main() {
    ...
    const uvec2 px = DispatchRaysIndex().xy * 2 + HALFRES_SUBSAMPLE_OFFSET;
    ...
    vec4 gbuffer_packed = gbuffer_tex[hi_px];
    ...
    const vec3 ray_orig_ws = ray_orig_history_tex[px].xyz + get_prev_eye_position();
    const vec3 ray_hit_ws = ray_history_tex[px].xyz + ray_orig_ws;
    ...
    const vec4 prev_irradiance_packed = irradiance_history_tex[px];
    const vec3 prev_irradiance = max(0.0.xxx, prev_irradiance_packed.rgb * frame_constants.pre_exposure_delta);
    ...
    irradiance_history_tex[px] = vec4(check_radiance, prev_irradiance_packed.a);
    ...
    // Also reduce M of the neighbors in case we have fewer validation rays than irradiance rays.
        ...
        for (uint i = 1; i <= 3; ++i) {
            ...
            const uvec2 px = DispatchRaysIndex().xy * 2 + hi_px_subpixels[(frame_constants.frame_index + i) & 3];

            const vec4 neighbor_prev_irradiance_packed = irradiance_history_tex[px];
            {
                const vec3 a = max(0.0.xxx, neighbor_prev_irradiance_packed.rgb * frame_constants.pre_exposure_delta);
                ...
                if (neigh_rad_diff < 0.2) {
                    ...
                    irradiance_history_tex[px] = vec4(check_radiance, neighbor_prev_irradiance_packed.a);
                }
            }
