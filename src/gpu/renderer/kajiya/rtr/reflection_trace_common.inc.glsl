...
RtrTraceResult do_the_thing(uvec2 px, vec3 normal_ws, float roughness, inout uint rng, RayDesc outgoing_ray) {
    ...
    if (!LAYERED_BRDF_FORCE_DIFFUSE_ONLY) {
        ...
        if (primary_hit.is_hit) {
            ...
            gbuffer.roughness = max(gbuffer.roughness, 1.0, roughness_bias);
            const float3x3 tangent_to_world = build_orthonormal_basis(gbuffer.normal);
            const vec3 wo = mul(-outgoing_ray.Direction, tangent_to_world);
            ...
            const vec3 primary_hit_cs = position_world_to_sample(primary_hit.position);
            ...
            vec3 reflected_normal_vs;
            {
                // Sun
                {
                    ...
                    const vec3 wi = mul(to_light_norm, tangent_to_world);

                    const vec3 brdf_value = brdf.evaluate(wo, wi) * max(0.0, wi.z);
                    const vec3 light_radiance = select(is_shadowed, 0.0, SUN_COLOR);
