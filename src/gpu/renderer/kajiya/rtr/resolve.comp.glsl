...
[[vk::binding(2)]] Texture2D<vec4> hit0_tex;
[[vk::binding(3)]] Texture2D<vec4> hit1_tex;
[[vk::binding(4)]] Texture2D<vec4> hit2_tex;
[[vk::binding(5)]] Texture2D<vec4> history_tex;
[[vk::binding(6)]] Texture2D<vec4> reprojection_tex;
[[vk::binding(7)]] Texture2D<vec4> half_view_normal_tex;
[[vk::binding(8)]] Texture2D<float> half_depth_tex;
[[vk::binding(9)]] Texture2D<vec2> ray_len_history_tex;
[[vk::binding(10)]] Texture2D<vec4> restir_irradiance_tex;
[[vk::binding(11)]] Texture2D<vec4> restir_ray_tex;
[[vk::binding(12)]] Texture2D<uvec2> restir_reservoir_tex;
[[vk::binding(13)]] Texture2D<vec4> restir_ray_orig_tex;
[[vk::binding(14)]] Texture2D<vec4> restir_hit_normal_tex;
[[vk::binding(15)]] RWTexture2D<vec3> output_tex;
[[vk::binding(16)]] RWTexture2D<vec2> ray_len_output_tex;
[[vk::binding(17)]] cbuffer _ {
    vec4 output_tex_size;
    ivec4 spatial_resolve_offsets[16 * 4 * 8];
};
...
[numthreads(8, 8, 1)]
void main(uvec2 px : SV_DispatchThreadID, uvec2 px_tile: SV_GroupID, uint idx_within_group: SV_GroupIndex) {
    //px = decode_morton_2d(idx_within_group) + px_tile * 8;
    //px = (px & ~3) | ((px & 1) << 1) | ((px & 2) >> 1);

    const uvec2 half_px = px / 2;

    const vec2 uv = get_uv(px, output_tex_size);
    ...
    const vec4 gbuffer_packed = gbuffer_tex[px];
    ...
    const vec3 refl_ray_origin_ws = view_ray_context.biased_secondary_ray_origin_ws_with_normal(gbuffer.normal);
#else
    ...
    const vec4 reprojection_params = reprojection_tex[px];

    const float ray_squish_scale = 16.0 / max(1e-5, eye_to_surf_dist);
    const float ray_len_avg = exponential_unsquish(max(
    ...
    const vec3 normal_vs = direction_world_to_view(gbuffer.normal);
    ...
    vec4 blue = blue_noise_for_pixel(half_px + 16, frame_constants.frame_index);
    ...
    for (int sample_i = 1; sample_i <= sample_count; ++sample_i, sample_radius_accum += RADIUS_INC_ON_FAIL) {
        ...
        ivec2 sample_offset; {
            ...
            vec3 sample_cs = position_world_to_sample(sample_ws);
            vec2 sample_uv = cs_to_uv(sample_cs.xy);

            // TODO: pass in `input_tex_size`
            ivec2 sample_px = ivec2(floor(sample_uv * output_tex_size.xy / 2));
            sample_offset = sample_px - half_px;
        }

        const ivec2 sample_px = half_px + sample_offset;

        // Only used in the non-ReSTIR path
        const float sample_depth = half_depth_tex[sample_px];

        // Only used in the non-ReSTIR path
        const vec4 packed1 = hit1_tex[sample_px];
        ...
        if (is_valid_sample) {
            ...
            vec3 sample_hit_normal_vs;    // only valid without ReSTIR
            if (!USE_RESTIR) {
                sample_hit_normal_vs = hit2_tex[sample_px].xyz;
            }

            #if CUT_CORNERS_IN_MATH
                const vec3 sample_normal_vs = half_view_normal_tex[sample_px].xyz;
            #else
                const vec3 sample_normal_vs = normalize(half_view_normal_tex[sample_px].xyz);
            #endif
            ...
            if (USE_RESTIR) {
                uvec2 rpx = sample_px;
                const uvec2 reservoir_raw = restir_reservoir_tex[rpx];
                ...
                const vec3 sample_hit_ws = restir_ray_tex[spx].xyz + sample_origin_ws;
                ...
                center_to_hit_vs = position_world_to_view(sample_hit_ws) - max(refl_ray_origin_vs, sample_origin_vs, RTR_NEIGHBOR_RAY_ORIGIN_CENTER_BIAS);
                center_to_hit_ws = sample_hit_ws - max(refl_ray_origin_ws, sample_origin_ws, RTR_NEIGHBOR_RAY_ORIGIN_CENTER_BIAS);
                ...
                if (RTR_USE_BULLSHIT_TO_FIX_EDGE_HALOS) {
                    ...
                    const float center_to_hit_dist_wat_i_dont_even = length(
                        ...
                        - max(refl_ray_origin_vs, sample_origin_vs, max(
                            1.0,
                    ...
                    vec3 wi = normalize(mul(direction_view_to_world(center_to_hit_vs), tangent_to_world));

                    const vec3 sample_origin_to_hit_vs = position_world_to_view(sample_hit_ws) - sample_origin_vs;
                ...
                const vec4 packed0 = hit0_tex[sample_px];
                ...
                const vec3 real_sample_hit_vs = direction_world_to_view(packed1.xyz) + sample_origin_vs;

                center_to_hit_vs = real_sample_hit_vs - max(refl_ray_origin_vs, sample_origin_vs, RTR_NEIGHBOR_RAY_ORIGIN_CENTER_BIAS);
                sample_hit_vs = center_to_hit_vs + refl_ray_origin_vs;
            }

            const vec3 wi = normalize(mul(direction_view_to_world(center_to_hit_vs), tangent_to_world));
    ...
    ray_len_output_tex[px] = vec2(ray_len_accum, ray_len_avg);
}
