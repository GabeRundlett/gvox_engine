...
ViewRayContext vrc_from_uv(daxa_RWBufferPtr(GpuGlobals) globals, vec2 uv) {
    ViewRayContext res;
...
ViewRayContext unjittered_vrc_from_uv(daxa_RWBufferPtr(GpuGlobals) globals, vec2 uv) {
    ViewRayContext res;
    res.ray_dir_cs = vec4(uv_to_cs(uv), 0.0, 1.0);
...
ViewRayContext vrc_from_uv_and_depth(daxa_RWBufferPtr(GpuGlobals) globals, vec2 uv, float depth) {
    ViewRayContext res;
    res.ray_dir_cs = vec4(uv_to_cs(uv), 0.0, 1.0);
    res.ray_dir_vs_h = deref(globals).player.cam.sample_to_view * res.ray_dir_cs;
    res.ray_dir_ws_h = deref(globals).player.cam.view_to_world * res.ray_dir_vs_h;
...
ViewRayContext vrc_from_uv_and_biased_depth(daxa_RWBufferPtr(GpuGlobals) globals, vec2 uv, float depth) {
    return vrc_from_uv_and_depth(globals, uv, min(1.0, depth * BIAS));
}

vec3 get_eye_position(daxa_RWBufferPtr(GpuGlobals) globals) {
...
vec3 get_prev_eye_position(daxa_RWBufferPtr(GpuGlobals) globals) {
    vec4 eye_pos_h = deref(globals).player.cam.prev_view_to_prev_world * vec4(0, 0, 0, 1);
    return eye_pos_h.xyz / eye_pos_h.w + deref(globals).player.player_unit_offset;
}
vec3 direction_view_to_world(daxa_RWBufferPtr(GpuGlobals) globals, vec3 v) {
    return (deref(globals).player.cam.view_to_world * vec4(v, 0)).xyz;
}
vec3 direction_world_to_view(daxa_RWBufferPtr(GpuGlobals) globals, vec3 v) {
    return (deref(globals).player.cam.world_to_view * vec4(v, 0)).xyz;
}
vec3 position_world_to_view(daxa_RWBufferPtr(GpuGlobals) globals, vec3 v) {
    return (deref(globals).player.cam.world_to_view * vec4(v, 1)).xyz;
}
vec3 position_view_to_world(daxa_RWBufferPtr(GpuGlobals) globals, vec3 v) {
    return (deref(globals).player.cam.view_to_world * vec4(v, 1)).xyz;
}

vec3 position_world_to_sample(daxa_RWBufferPtr(GpuGlobals) globals, vec3 v) {
    vec4 p = deref(globals).player.cam.world_to_view * vec4(v, 1);
    p = deref(globals).player.cam.view_to_sample * p;
    return p.xyz / p.w;
}

vec3 position_world_to_clip(daxa_RWBufferPtr(GpuGlobals) globals, vec3 v) {
    vec4 p = (deref(globals).player.cam.world_to_view * vec4(v, 1));
