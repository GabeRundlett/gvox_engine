...
DAXA_TASK_USE_IMAGE(half_depth_image, REGULAR_2D, COMPUTE_SHADER_READ)
...
struct SsaoComputeTaskState {
    ...
    SsaoComputeTaskState(daxa::PipelineManager &a_pipeline_manager, AppUi &a_ui, u32vec2 &a_render_size) : pipeline_manager{a_pipeline_manager}, ui{a_ui}, render_size{a_render_size} {}
    ...
    void record_commands(daxa::CommandList &cmd_list) {
        ...
        assert((render_size.x % 16) == 0 && (render_size.y % 16) == 0);
        cmd_list.dispatch(render_size.x / 16, render_size.y / 16);
    }
};
