...
#define GLSLANG_WRAPPER_INTERNAL 1
#define GLSLANG_WRAPPER_DLL_EXPORT extern "C" __declspec(dllexport)
...
public:
	...
	const GlslangWrapperCompileInfo* info;

	auto includeLocal(
		const char* header_name, const char* includer_name, size_t inclusion_depth) -> IncludeResult* override
	{
...
GLSLANG_WRAPPER_DLL_EXPORT void glslang_wrapper_compile(const GlslangWrapperCompileInfo& info)
{
	...
	const TBuiltInResource resource = DAXA_DEFAULT_BUILTIN_RESOURCE;
