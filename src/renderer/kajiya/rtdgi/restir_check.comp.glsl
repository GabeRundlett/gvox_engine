...
[[vk::binding(1)]] Texture2D<uvec4> temporal_reservoir_packed_tex;
[[vk::binding(2)]] RWTexture2D<uvec2> reservoir_input_tex;
...
void main() {
    ...
    const vec3 spx_pos_ws = spx_ray_ctx.ray_hit_ws();

    const vec3 trace_origin_ws =
        //view_ray_context.ray_hit_ws();
        view_ray_context.biased_secondary_ray_origin_ws();

    const vec3 trace_vec = hit_ws - trace_origin_ws;
