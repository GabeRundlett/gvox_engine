...
[[vk::binding(2)]] Texture2D<vec4> reprojection_tex;
[[vk::binding(3)]] Texture2D<vec2> closest_velocity_tex;
[[vk::binding(4)]] Texture2D<vec2> velocity_history_tex;
[[vk::binding(5)]] Texture2D<float> depth_tex;
[[vk::binding(6)]] Texture2D<vec3> smooth_var_history_tex;
[[vk::binding(7)]] Texture2D<float> input_prob_tex;
[[vk::binding(8)]] RWTexture2D<vec4> temporal_output_tex;
[[vk::binding(9)]] RWTexture2D<vec4> output_tex;
[[vk::binding(10)]] RWTexture2D<vec3> smooth_var_output_tex;
[[vk::binding(11)]] RWTexture2D<vec2> velocity_output_tex;
...
vec4 fetch_blurred_history(ivec2 px, int k, float sigma) {
    const vec3 center = history_tex[px].rgb;
    ...
    for (int y = -k; y <= k; ++y) {
        for (int x = -k; x <= k; ++x) {
            vec4 c = history_tex[px + ivec2(x, y)];
...
void main(uvec2 px: SV_DispatchThreadID) {
    ...
    const vec2 input_resolution_fraction = input_tex_size.xy / output_tex_size.xy;
    const uvec2 reproj_px = uvec2((px + 0.5) * input_resolution_fraction);
    //const uvec2 reproj_px = uvec2(px * input_resolution_fraction + 0.5);

    #if SHORT_CIRCUIT
        temporal_output_tex[px] = lerp(input_tex[reproj_px], vec4(encode_rgb(history_tex[px].rgb), 1), 1.0 - 1.0 / SHORT_CIRCUIT);
    ...
    vec4 history_packed = history_tex[px];
    ...
    vec3 bhistory_coverage = bhistory_packed.a;

    history = sRGB_to_YCbCr(history);
    bhistory = sRGB_to_YCbCr(bhistory);

    const vec4 reproj = reprojection_tex[reproj_px];
    const vec2 reproj_xy = closest_velocity_tex[px];
...
#else
    vec3 center = sRGB_to_YCbCr(decode_rgb(INPUT_TEX[px].rgb));
#endif
    ...
    const vec3 prev_var = smooth_var_history_tex.SampleLevel(sampler_lnc, uv + reproj_xy, 0).x;

    // TODO: factor-out camera-only velocity
    const vec2 vel_now = closest_velocity_tex[px] / frame_constants.delta_time_seconds;
    const vec2 vel_prev = velocity_history_tex.SampleLevel(sampler_llc, uv + closest_velocity_tex[px], 0);
    ...
    vec3 smooth_var = max(var, lerp(prev_var, var, var_blend));
    ...
    vec4 this_frame_result = 0;
    #define DEBUG_SHOW(value) { this_frame_result = vec4((vec3)(value), 1); }
    ...
    // Perform neighborhood clamping / disocclusion rejection
    {
    	...
    	vec3 nmax = ex + input_dev * box_n_deviations;
            ...
            vec3 outlier3 = max(0.0, (max(nmin - history, history - nmax)) / (0.1 + max(max(abs(history), abs(ex)), 1e-5)));
            vec3 boutlier3 = max(0.0, (max(nmin - bhistory, bhistory - nmax)) / (0.1 + max(max(abs(bhistory), abs(ex)), 1e-5)));
    ...
    temporal_output_tex[px] = vec4(temporal_result, coverage);
