...
void main() {
    ...
    vec3 input_tex_value = texelFetch(daxa_texture2D(input_tex), ivec2(px), 0).rgb / deref(gpu_input).pre_exposure;
    float lum = sRGB_to_luminance(input_tex_value);
    if (isnan(lum))
        lum = 0;
    float log_lum = log2(max(1e-10, lum));
